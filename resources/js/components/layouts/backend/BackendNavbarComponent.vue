        ...
        <div class="flex items-center justify-end w-full gap-4">
            <div
                ...
                <div class="flex items-center justify-between md:justify-center gap-4">
                    ...
                    <button
                        @click.prevent="openHoldOrdersInPos"
                        class="flex items-center gap-2 h-9 px-3 rounded-lg bg-[#008BBA] text-white text-xs font-medium whitespace-nowrap"
                    >
                        <i class="lab lab-clock"></i>
                        <span>HOLD ORDERS ({{ navHoldOrdersCount }})</span>
                    </button> <!-- extra hold order -->
...
export default {
    ...
    mounted() {
        ...
        window.addEventListener('hold-orders-updated', this.refreshHoldCount);
    },
    methods: {
        ...
        refreshHoldCount: function () {
            try {
                ...
                if (saved) {
                    ...
                    this.navHoldOrdersCount = all.filter(o => new Date(o.timestamp) >= today).length;
                } else {
                    this.navHoldOrdersCount = 0;
                }
            } catch (e) {
                this.navHoldOrdersCount = 0;
            }
        ...
        openHoldOrdersInPos: function () {
            window.dispatchEvent(new Event('pos-open-hold-orders'));
        },
